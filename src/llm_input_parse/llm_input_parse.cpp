...
parseInputFromJson(Json const& inputData, int32_t batchSizeOverride, int64_t maxGenerateLengthOverride)
{
    std::vector<trt_edgellm::rt::LLMGenerationRequest> batchedRequests;
    ...
    if (inputData.contains("requests") && inputData["requests"].is_array())
    {
        ...
        {
            trt_edgellm::rt::LLMGenerationRequest batchRequest;
            ...
            for (size_t requestIdx = startIdx; requestIdx < endIdx; ++requestIdx)
            {
                ...
                std::vector<trt_edgellm::rt::Message> chatMessages;
                std::vector<trt_edgellm::rt::imageUtils::ImageData> imageBuffers;
                std::vector<trt_edgellm::rt::audioUtils::AudioData> audioBuffers;
                ...
                for (auto const& messageJson : messagesArray)
                {
                    ...
                    trt_edgellm::rt::Message chatMsg;
                    ...
                    if (contentJson.is_string())
                    {
                        ...
                        trt_edgellm::rt::Message::MessageContent msgContent;
                    ...
                    else if (contentJson.is_array())
                    {
                        ...
                        for (auto const& contentItemJson : contentJson)
                        {
                            ...
                            else if (msgContent.type == "image")
                            {
                                ...
                                auto image = trt_edgellm::rt::imageUtils::loadImageFromFile(msgContent.content);
                            ...
                            else if (msgContent.type == "audio")
                            {
                                ...
                                trt_edgellm::rt::audioUtils::AudioData audio;
                                std::string audioPath = msgContent.content;
                                size_t dotPos = audioPath.find_last_of('.');
                                std::string extension = (dotPos != std::string::npos) ? audioPath.substr(dotPos) : "";
                                ...
                                {
                                    LOG_WARNING("Unsupported audio format: %s (only .safetensors supported)",
                                        audioPath.c_str());
                                }
                            }
                            else
                ...
                trt_edgellm::rt::LLMGenerationRequest::Request request;
...
std::pair<std::unordered_map<std::string, std::string>, std::vector<trt_edgellm::rt::LLMGenerationRequest>>
parseInputFile(std::filesystem::path const& inputFilePath, int32_t batchSizeOverride, int64_t maxGenerateLengthOverride)
{
    Json inputData;
    ...
    return parseInputFromJson(inputData, batchSizeOverride, maxGenerateLengthOverride);
}
